...
//判断二叉树是否对称
/* 由于要求树有对称性，也就是以根节点为中心，两边对称，
所以比较的是同一根节点的左右子树是否相同。即左子树的遍历顺序与右子树的遍历顺序一样。 */
...
/* template<class T>
bool check(BinTreeNode<T> *l,BinTreeNode<T> *r){
    if(l==NULL && r==NULL) return true;
    if(((l==NULL||r==NULL)&&l!=r)||l->data!=r->data) return false;  //左右根节点不同或其中一个为空
    return check(l->leftChild, r->rightChild)&&check(l->rightChild, r->leftChild);      //将对称的两个根节点进行遍历
  }  

template<class T>
bool isSymmetric (BinTreeNode<T> *root) {
...
bool isSymmetric(BinTreeNode<T> * root) {
    ...
    //用两个指针检查树是否对称
	template<class T>
bool check(BinTreeNode<T> * p, BinTreeNode<T> * q){
...
int main()
{
	...
	if(a==1)
	{cin>>t;
   cout<<0<<" true"; return 0;
	}
	/* cout << "请按照节点顺序输入" << a << "个二叉树数据" << endl; */
	BinaryTree<string> *tree = new BinaryTree<string>;//这里有null 用string方便 虽然有点慢
	tree->LevelCreateTree(a);
	//	cout << endl << "二叉树的高度:" << endl;
    if(isSymmetric(tree->getRoot())==true) cout << tree->Height()-1 <<" true";
	else cout << tree->Height()-1 <<" false";
